...
eine offene Umgebung von $\Homoeo(x)$ (siehe \Cref{defn:schwachSternTop}), so gilt für $y \in \tau^{-1}(B_\epsilon(\tau(x))) \subseteq X$ (eine offene Menge, da $\tau \in \stetig(X)$, also stetig):
...
Somit ist $\Homoeo: X \to \Homoeo(X)$ stetig und bijektiv. Ferner ist $X$ kompakt und $\Homoeo(X) \subseteq \SpecC(\stetig(X))$ hausdorffsch (\Cref{lemma:MA}). Damit ist $\Homoeo: X \to \Homoeo(X)$ nach \Cref{lemma:komHDHomoeo} bereits ein Homöomorphismus.
